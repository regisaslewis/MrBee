import random
from player_guess_sql import player_guess, show_points, read_guesses, today
...
def main():
    ...
    while True:
        ...
        if player_choice == "0":
            ...
            print(f"{show_points()} of {win_condition} points in {len(read_guesses())} word{'' if len(read_guesses()) == 1 else 's'}:")
            print("None yet" if len(read_guesses()) == 0 else sorted(read_guesses()))
            print("=============")
